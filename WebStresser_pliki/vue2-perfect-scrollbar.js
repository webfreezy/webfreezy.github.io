(function(global,factory){typeof exports==='object'&&typeof module!=='undefined'?factory(exports):typeof define==='function'&&define.amd?define(['exports'],factory):(factory((global.Vue2PerfectScrollbar={})));}(this,(function(exports){'use strict';/*!
...
function processScrollDiff(i,axis,diff,useScrollingClass,forceFireReachEvent){if(useScrollingClass===void 0){useScrollingClass=true;}
if(forceFireReachEvent===void 0){forceFireReachEvent=false;}
var fields;if(axis==='top'){fields=['contentHeight','containerHeight','scrollTop','y','up','down'];}else if(axis==='left'){fields=['contentWidth','containerWidth','scrollLeft','x','left','right'];}else{throw new Error('A proper axis should be provided');}
processScrollDiff$1(i,diff,fields,useScrollingClass,forceFireReachEvent);}
function processScrollDiff$1(i,diff,ref,useScrollingClass,forceFireReachEvent){var contentHeight=ref[0];var containerHeight=ref[1];var scrollTop=ref[2];var y=ref[3];var up=ref[4];var down=ref[5];if(useScrollingClass===void 0){useScrollingClass=true;}
if(forceFireReachEvent===void 0){forceFireReachEvent=false;}
var element=i.element;i.reach[y]=null;if(element[scrollTop]<1){i.reach[y]='start';}
...
this.element=element;element.classList.add(cls.main);this.settings=defaultSettings();for(var key in userSettings){this$2.settings[key]=userSettings[key];}
...
this.event.unbindAll();remove(this.scrollbarX);remove(this.scrollbarY);remove(this.scrollbarXRail);remove(this.scrollbarYRail);this.removePsClasses();this.element=null;this.scrollbarX=null;this.scrollbarY=null;this.scrollbarXRail=null;this.scrollbarYRail=null;this.isAlive=false;};PerfectScrollbar.prototype.removePsClasses=function removePsClasses(){this.element.className=this.element.className.split(' ').filter(function(name){return!name.match(/^ps([-_].+|)$/);}).join(' ');};var PerfectScrollbar$1={name:'PerfectScrollbar',props:{options:{type:Object,required:false,default:function(){}},tag:{type:String,required:false,default:'div'},watchOptions:{type:Boolean,required:false,default:false}},data:function data(){return{ps:null}},watch:{watchOptions:function watchOptions(shouldWatch){if(!shouldWatch&&this.watcher){this.watcher();}else{this.createWatcher();}}},mounted:function mounted(){this.create();if(this.watchOptions){this.createWatcher();}},updated:function updated(){var this$1=this;this.$nextTick(function(){this$1.update();});},beforeDestroy:function beforeDestroy(){this.destroy();},methods:{create:function create(){if(!(this.ps&&this.$isServer)){this.ps=new PerfectScrollbar(this.$refs.container,this.options);}},createWatcher:function createWatcher(){var this$1=this;this.watcher=this.$watch('options',function(){this$1.destroy();this$1.create();},{deep:true});},update:function update(){if(this.ps){this.ps.update();}},destroy:function destroy(){if(this.ps){this.ps.destroy();this.ps=null;}}},render:function render(h){return h(this.tag,{ref:'container',class:'ps',on:this.$listeners},this.$slots.default)}};function install(Vue,settings){if(settings){if(settings.name&&typeof settings.name==='string'){PerfectScrollbar$1.name=settings.name;}
if(settings.options&&typeof settings.options==='object'){PerfectScrollbar$1.props.options.default=function(){return settings.options};}
if(settings.tag&&typeof settings.tag==='string'){PerfectScrollbar$1.props.tag.default=settings.tag;}
if(settings.watchOptions&&typeof settings.watchOptions==='boolean'){PerfectScrollbar$1.props.watchOptions=settings.watchOptions;}}
Vue.component(PerfectScrollbar$1.name,PerfectScrollbar$1);}
exports.install=install;exports.PerfectScrollbar=PerfectScrollbar$1;exports.default=install;Object.defineProperty(exports,'__esModule',{value:true});})));